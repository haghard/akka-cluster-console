...
object ClusterViewModule extends GraphSupport {
  ...
  class GraphBackend(scope: BackendScope[ConsoleProps, ConsoleState]) extends OnUnmount {
    ...
    private def parse(m: scala.scalajs.js.Dictionary[scala.scalajs.js.Any]): Option[ClusterMember] = {
      ...
      val rolesJs = m("roles").asInstanceOf[scala.scalajs.js.Array[scala.scalajs.js.Any]]
      val roles   = rolesJs.filter(_.toString != "dc-default").map(_.toString).toSet

      n match {
        case Exp(sysName, a, b, c, d, port) =>
          val hp = HostPort(s"${a}:${b}:${c}:${d}", port.toInt)
          Some(ClusterMember(hp, roles, NodeState.Up))
        case _ =>
          None
      }
    }
        ...
        org.scalajs.dom.ext.Ajax
          ...
          .map { resp =>
            ...
            (json("leader").toString match {
              case Exp(systemName, a, b, c, d, systemPort) =>
                Some(systemName)
              case _ =>
                None
            }).map { sn =>
              ClusterProfile(sn, members.map(parse(_)).flatten.toSet)
            }.fold {
              scope.modState(s => s.copy(system = Option("cluster-profile json parse error")))
            } { cProfile =>
              dom.console.log(s"cProfile: $cProfile")
              val location = 10

              val hosts: Set[Vertix] = cProfile.members.map(_.address.host).zipWithIndex.map { case (address, i) =>
                lit(
                  "id"     -> i,
                  "x"      -> (location * (i + 1)),
                  "y"      -> 40,
              ...
              val members: Set[Vertix] = cProfile.members.zipWithIndex.map { case (m, ind) =>
                lit(
          ...
          .recoverWith { case ex: org.scalajs.dom.ext.AjaxException =>
            Future.successful {
   ...
   def onChange2(s: Selection[org.scalajs.dom.EventTarget], force: forceModule.Force[Vertix, Edge]): Unit = {
    ...
    private def onChange(s: Selection[org.scalajs.dom.EventTarget], force: forceModule.Force[Vertix, Edge]): Unit = {
      ...
      val nodeEnter = s
        ...
        .data[Vertix](force.nodes(), (n: Vertix, i: Int) => n.id.toString)
        .enter()
        .append("g")
        .attr("class", (n: Vertix) => s"node ${n.id}")
        .call(force.drag())
      ...
      nodeEnter
        ...
        .attr(
          "xlink:href",
          // (n: Vertix) => if (!n.isHost) "console-assets/akka.jpg" else "console-assets/akka-small.jpg"
          (n: Vertix) => "pics/akka-small.jpg"
        )
        .attr("width", "60px")
        .attr("height", "40px")
      ...
      /*nodeEnter
        ...
        .attr("y", 15)
        .text((n: Vertix) => if (n.isHost) n.host else s"Port:${n.port}")
       */
