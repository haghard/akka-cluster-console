...
object Graph5 {
  ...
  def create(scene: String) = {
    ...
    val root = Dynamic.literal("id" -> 1.0, "index" -> 1.0, "name" -> "192", "x" -> 50, "y" -> 50,
      "host" -> "192", "port" -> 1, "roles" -> "ws", "status" -> "up", "size" -> 1).asInstanceOf[N]
    ...
    def onTick(e: org.scalajs.dom.Event, s: Selection[org.scalajs.dom.EventTarget]): Unit = {
      ...
      s.selectAll[N](".node")
        .attr("r", { (n: N) => 10 + (n.size) }) //if you have a big number of nodes /*Math.sqrt(n.size)*/
        .attr("cx", { (n: N) => n.x })
        .attr("cy", { (n: N) => n.y })
    }
    ...
    (1 to num).foreach { i =>
      d3.timer(() => {
        ...
        val nodeUpdate = svg.selectAll[N](".node")
          .data[N](force.nodes(), { (d: N, i: Int) => d.id.toString })
        ...
        val linkUpdate = svg.selectAll[Edge](".link").
