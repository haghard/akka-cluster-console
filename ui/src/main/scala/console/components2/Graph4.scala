...
object Graph4 {
  ...
  def render(s: Selection[org.scalajs.dom.EventTarget], force: Force[N, Edge]) = {
    ...
    nodeEnter
      .append("circle")
      .attr("r", { (n: N) ⇒
        if (js.isUndefined(n.parent)) 20 else 5
      })
      //{ (n:N) => 10 + n.size })
  ...
  def create(scene: String) = {
    val host0 = Dynamic
      .literal(
        ...
        "x"      → 50,
        "y"      → 50,
        "host"   → shared.Network.toInetAddress(178493535),
        "port"   → 1,
        "roles"  → "ws",
        "status" → "up",
        "size"   → 1
    ...
    val host1 = Dynamic
      .literal(
        ...
        "x"      → 50,
        "y"      → 50,
        "host"   → shared.Network.toInetAddress(178493536),
        "port"   → 1,
        "roles"  → "ws",
        "status" → "up",
        "size"   → 1
