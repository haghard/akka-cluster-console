...
object Graph3 {
  ...
  def create(scene: String) = {
    ...
    def onTick(e: org.scalajs.dom.Event): Unit = {
      nodeSelection
        .attr("cx", { (n: CgraphNode) => n.x })
        .attr("cy", { (n: CgraphNode) => n.y })
    ...
    val force = d3.layout.force[CgraphNode, Link]()
      .nodes(nodes)
      .links(links)
      .charge(-400)
      .linkDistance(200)
      .size((width, height))
      .on("tick", onTick(_))
      .on("end", { e: org.scalajs.dom.Event => println("calculations have concluded") })

    val svg = d3.select(scene)
      .append("svg")
      .attr("width", width)
      .attr("height", height)
    ...
    setTimeout(0) {
      val a = Dynamic.literal("id" -> 1.0, "index" -> 1.0, "name" -> "192", "x" -> 50, "y" -> 50,
        "host" -> "192", "port" -> 1, "roles" -> "ws", "status" -> "up").asInstanceOf[CgraphNode]
      val b = Dynamic.literal("id" -> 2.0, "index" -> 2.0, "name" -> "193", "x" -> 60, "y" -> 60,
        "host" -> "193", "port" -> 2, "roles" -> "ws", "status" -> "up").asInstanceOf[CgraphNode]
      val c = Dynamic.literal("id" -> 3.0, "index" -> 3.0, "name" -> "194", "x" -> 70, "y" -> 70,
        "host" -> "194", "port" -> 3, "roles" -> "ws", "status" -> "up").asInstanceOf[CgraphNode]
    ...
    def render() = {
      linkSelection = svg.selectAll[Link](".linkSelection")
      ...
      nodeSelection = svg.selectAll[CgraphNode](".node")
        .data(force.nodes(), { (d: CgraphNode, i: Int) => d.id.toString })
